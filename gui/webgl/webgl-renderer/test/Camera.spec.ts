...
describe('Camera', function () {
  describe('Culling', function () {
    it('Gets bounds correctly', function () {
      // Whole numbers
      const camera = new Camera(new Vector(0, 0), 8, new Vector(128, 128));
      let [bottomLeft, topRight] = camera.getBounds();
      expect(bottomLeft.equals(new Vector(0, 0))).to.be.true;
      expect(topRight.equals(new Vector(7, 7))).to.be.true;
      camera.setZoom(16);
      [bottomLeft, topRight] = camera.getBounds();
      expect(bottomLeft.equals(new Vector(0, 0))).to.be.true;
      ...
      camera.setZoom(10);
      camera.viewport = new Vector(100, 100);
      [bottomLeft, topRight] = camera.getBounds();
      expect(bottomLeft.equals(new Vector(0, 0))).to.be.true;
      ...
      camera.setZoom(10);
      camera.viewport = new Vector(1000, 1000);
      [bottomLeft, topRight] = camera.getBounds();
      expect(bottomLeft.equals(new Vector(0, 0))).to.be.true;
      expect(topRight.equals(new Vector(100, 100))).to.be.true;
    });

    it.skip('Culls correctly', function () {
